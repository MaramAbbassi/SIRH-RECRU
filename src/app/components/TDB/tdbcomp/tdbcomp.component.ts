...
@Component({
  selector: 'app-tdbcomp',
  templateUrl: './tdbcomp.component.html',
  styleUrls: ['./tdbcomp.component.css']
})
export class TdbcompComponent implements OnInit {
  ...
  onDelete(num: number){
    this.auth.deleteUser(num).subscribe(res => {
      this.api.getAllTsirhUsers();
      this.signUpForm.reset();
      window.location.reload();
    })
